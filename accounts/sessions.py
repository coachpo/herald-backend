...
_RefreshTokenModel = cast(Any, RefreshToken)
_atomic = cast(Any, transaction.atomic)
...
def create_refresh_token(
    *, user: User, ip: str | None, user_agent: str | None
    ...
    rt = _RefreshTokenModel.objects.create(
        user=user,
...
def rotate_refresh_token(
    *, token_hash: str, ip: str | None, user_agent: str | None
    ...
    with _atomic():
        now = timezone.now()
        try:
            current = _RefreshTokenModel.objects.select_for_update().get(
                token_hash=token_hash
            )
        except _RefreshTokenModel.DoesNotExist as e:
            raise ValueError("invalid_refresh") from e

        if current.revoked_at is not None:
            _RefreshTokenModel.objects.filter(
        ...
        raw, new_rt = create_refresh_token(
            user=current.user, ip=ip, user_agent=user_agent
        )
...
def revoke_all_refresh_tokens(*, user: User, reason: str):
    now = timezone.now()
    _RefreshTokenModel.objects.filter(user=user, revoked_at__isnull=True).update(
        revoked_at=now,
        revoked_reason=reason,
    )
